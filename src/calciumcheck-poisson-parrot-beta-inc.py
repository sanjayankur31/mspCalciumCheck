...
for rate in numpy.arange(5, 50, 5, dtype=float):
    nest.ResetKernel()
    neuron_dict = {'beta_Ca': 0.012, 'tau_Ca': 10000.0}
    ...
    plt.legend()
    plt.savefig("parrot-calcium-beta-inc-data-{}Hz.png".format(rate))
    plt.close(fig)
...
plt.savefig("parrot-calcium-beta-inc-data-combined.png".format(rate))
plt.close(fig2)
